...
const solution = getSolutionBySlug("charger-installation")!;
const related = solutions.filter((s) => s.slug !== solution.slug);

export const metadata: Metadata = {
  title: solution.metaTitle,
  description: solution.metaDescription,
  alternates: { canonical: "/charging-solutions/charger-installation" },
};

export default function ChargerInstallationPage() {
  return (
    <main className="bg-[#03110d]">
      <Breadcrumbs
        items={[
          { name: "Charging Solutions", url: "/charging-solutions" },
          { name: solution.title, url: "/charging-solutions/charger-installation" },
        ]}
      />
      <PageHero eyebrow="Charger Installation" title={solution.title} description={solution.metaDescription} />
            ...
            {solution.highlights && (
              <ul className="grid gap-3 sm:grid-cols-2">
                {solution.highlights.map((item) => (
                  <li
                    key={item}
                    className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white/85"
                  >
